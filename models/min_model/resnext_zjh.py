...
class sta(nn.Module):
  ...
  def __init__(self,input_shape,r=256):  # r=32 or 256
    ...
    print('-----------min---------------')
    self.At1 = torch.nn.Parameter(torch.FloatTensor(nn.init.constant_(torch.Tensor(self.c,1),1.0/(self.c))), requires_grad=True)# input:0.1~,0.5*I*c=0.1
  ...
  def forward(self,sta_input: Tensor)-> Tensor:
    ...
    if not self.serial:
        ...
        input_t1 = torch.min(input_t, 3)[0]#(b,l,c)
        ...
        input_t2 = torch.min(input_t, 2)[0]#(b,l,hw)
        ...
        # add
        # input_t = input_t1 + input_t2 + input_t3
        # multiply
        ...
        input_c1 = torch.min(input_c, 3)[0]#(b,c,l)
        ...
        input_c2 = torch.min(input_c, 2)[0]#(b,c,hw)
        ...
        # add
        # input_c = input_c = input_c1 + input_c2 + input_c3
        # multiply
        ...
        input_s1 = torch.min(input_s, 3)[0]#(b,hw,l)
        ...
        input_s2 = torch.min(input_s, 2)[0]#(b,hw,c)
        ...
        # add
        # input_s = input_s1 + input_s2 + input_s3
        # multiply
        input_s = torch.mul(torch.mul(input_s1,input_s2),input_s3).reshape([self.b,1,1,self.h,self.w]) # (b,1,1,h,w)
