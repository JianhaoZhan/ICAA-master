...
class sta(nn.Module):
  ...
  def __init__(self,input_shape,r=256):  # r=32 or 256
    ...
    self.At3 = torch.nn.Parameter(torch.FloatTensor(nn.init.constant_(torch.Tensor(self.c*self.h*self.w,1),0.25/(self.c*self.h*self.w))), requires_grad=True)# input:0.5~
    ...
    self.Ac3 = torch.nn.Parameter(torch.FloatTensor(nn.init.constant_(torch.Tensor(self.l*self.h*self.w,1),0.25/(self.l*self.h*self.w))), requires_grad=True)# input:0.5~
    ...
    self.As3 = torch.nn.Parameter(torch.FloatTensor(nn.init.constant_(torch.Tensor(self.l*self.c,1),0.25/(self.l*self.c))), requires_grad=True)# input:0.5~,0.5*I*l*c=0.1
    self.Es1 = torch.nn.Parameter(torch.FloatTensor(nn.init.constant_(torch.Tensor(self.l*self.h*self.w,self.h*self.w),1.0/(self.h*self.w))), requires_grad=True)
    self.Es2 = torch.nn.Parameter(torch.FloatTensor(nn.init.constant_(torch.Tensor(self.l*self.h*self.w,self.h*self.w),1.0/(self.h*self.w))), requires_grad=True)
    self.Es3 = torch.nn.Parameter(torch.FloatTensor(nn.init.constant_(torch.Tensor(self.l*self.h*self.w,self.h*self.w),1.0/(self.h*self.w))), requires_grad=True)
    self.Ss1 = torch.nn.Parameter(torch.FloatTensor(nn.init.constant_(torch.Tensor(self.h*self.w,self.l*self.h*self.w),1.0/(self.l*self.h*self.w))), requires_grad=True)
    self.Ss2 = torch.nn.Parameter(torch.FloatTensor(nn.init.constant_(torch.Tensor(self.h*self.w,self.l*self.h*self.w),1.0/(self.l*self.h*self.w))), requires_grad=True)
    self.Ss3 = torch.nn.Parameter(torch.FloatTensor(nn.init.constant_(torch.Tensor(self.h*self.w,self.l*self.h*self.w),1.0/(self.l*self.h*self.w))), requires_grad=True)
  ...
  def forward(self,sta_input: Tensor)-> Tensor:
    ...
    if not self.serial:
        ...
        # add
        # input_t = torch.div(input_t1,1.5) + torch.div(input_t2,1.5) + torch.div(input_t3,1.5)
        # multiply
        input_t = torch.mul(torch.mul(input_t1,input_t2), input_t3)
        ...
        # add
        # input_c = torch.div(input_c1,1.5) + torch.div(input_c2,1.5) + torch.div(input_c3,1.5)
        # multiply
        input_c = torch.mul(torch.mul(input_c1,input_c2),input_c3)
        ...
        input_s1 = input_s1.squeeze(-1).unsqueeze(1).unsqueeze(1).reshape([self.b,1,1,self.h,self.w])
        ...
        input_s2 = input_s2.squeeze(-1).unsqueeze(1).unsqueeze(1).reshape([self.b,1,1,self.h,self.w])
        ...
        input_s3 = input_s3.squeeze(-1).unsqueeze(1).unsqueeze(1).reshape([self.b,1,1,self.h,self.w])
        # add
        # input_s = torch.div(input_s1,1.5) + torch.div(input_s2,1.5) + torch.div(input_s3,1.5)
        # multiply
        input_s = torch.mul(torch.mul(input_s1,input_s2),input_s3) # (b,1,1,h,w)
        input_s = torch.div(input_s,input_s.mean())
        ...
        input_t1 = self.relu(torch.matmul(self.Et1,input_t1))# self.relu(torch.div(torch.matmul(self.Et1,input_t1),self.l)) 
        input_t1 = self.sigmoid(torch.matmul(self.St1,input_t1))# self.sigmoid(torch.div(torch.matmul(self.St1,input_t1),self.r*self.l))
        #print('test T-1 for sigmoid:{}'.format(input_t1))
        # T2 (b,l,c,hw)->(b,l,hw){*At2(hw,1)}->{Et2(rl,l)*}(b,l,1)->{St2(l,rl)*}(b,rl,1)->(b,l,1)
        input_t2 = torch.mean(input_t, 2, False)#(b,l,hw)
        input_t2 = torch.matmul(input_t2,self.At2)# torch.div(torch.matmul(input_t2,self.At2),self.h*self.w)
        input_t2 = self.relu(torch.matmul(self.Et2,input_t2))# self.relu(torch.div(torch.matmul(self.Et2,input_t2),self.l))
        input_t2 = self.sigmoid(torch.matmul(self.St2,input_t2))# self.sigmoid(torch.div(torch.matmul(self.St2,input_t2),self.r*self.l))
        #print('test T-2 for sigmoid:{}'.format(input_t1))
        # T3 (b,l,c,hw)->(b,l,chw){*At3(chw,1)}->{Et3(rl,l)*}(b,l,1)->{St3(l,rl)*}(b,rl,1)->(b,l,1)
        input_t3 = input_t.reshape([self.b,self.l,self.c*self.h*self.w])#(b,l,chw)
        input_t3 = torch.matmul(input_t3,self.At3)# torch.div(torch.matmul(input_t3,self.At3),self.c*self.h*self.w)
        input_t3 = self.relu(torch.matmul(self.Et3,input_t3))# self.relu(torch.div(torch.matmul(self.Et3,input_t3),self.l)) 
        '''######
        print('test:para:At1:{},Et1:{},St1:{}'.format(self.At1,self.Et1,self.St1))
        print('test:para:At2:{},Et2:{},St2:{}'.format(self.At2,self.Et2,self.St2))
        print('test:para:At3:{},Et3:{},St3:{}'.format(self.At3,self.Et3,self.St3))
        print('test for sigmoid:{}'.format(torch.matmul(self.St3,input_t3)))
        exit()
        ######'''
        input_t3 = self.sigmoid(torch.matmul(self.St3,input_t3))# self.sigmoid(torch.div(torch.matmul(self.St3,input_t3),self.r*self.l))
        #print('test T-3 for sigmoid:{}'.format(input_t1))
        # add
        # input_t = torch.div(input_t1,1.5) + torch.div(input_t2,1.5) + torch.div(input_t3,1.5)
        # multiply
        input_t = torch.mul(torch.mul(input_t1,input_t2), input_t3)
        input_t = torch.div(input_t,torch.mean(input_t,1,True))
        #print('t',input_t),exit()
        # (b,l,1)->(b,1,l,1,1)
        input_t = input_t.unsqueeze(1).unsqueeze(-1)
        if input_t.shape != (self.b, 1, self.l, 1, 1):
            raise ValueError('the shape of temporal data is error !')
        ...
        input_c1 = self.relu(torch.matmul(self.Ec1,input_c1)) #self.relu(torch.div(torch.matmul(self.Ec1,input_c1),self.c))
        input_c1 = self.sigmoid(torch.matmul(self.Sc1,input_c1))# self.sigmoid(torch.div(torch.matmul(self.Sc1,input_c1),int(self.c/self.r)))
        #print('test C-1 for sigmoid:{}'.format(input_c1))
        # C2 (b,c,l,hw)->(b,c,hw){*Ac2(hw,1)}->{Ec2(c/r,c)*}(b,c,1)->{Sc2(c,c/r)*}(b,c/r,1)->(b,c,1)
        input_c2 = torch.mean(input_c, 2, False)#(b,c,hw)
        input_c2 = torch.matmul(input_c2,self.Ac2)# torch.div(torch.matmul(input_c2,self.Ac2),self.h*self.w)
        input_c2 = self.relu(torch.matmul(self.Ec2,input_c2))# self.relu(torch.div(torch.matmul(self.Ec2,input_c2),self.c))
        input_c2 = self.sigmoid(torch.matmul(self.Sc2,input_c2))# self.sigmoid(torch.div(torch.matmul(self.Sc2,input_c2),int(self.c/self.r)))
        #print('test C-2 for sigmoid:{}'.format(input_c1))
        # C3 (b,c,l,hw)->(b,c,lhw){*Ac3(lhw,1)}->{Ec3(c/r,c)*}(b,c,1)->{Sc3(c,c/r)*}(b,c/r,1)->(b,c,1)
        input_c3 = sta_input.reshape([self.b,self.c,self.l*self.h*self.w])#(b,c,lhw)
        input_c3 = torch.matmul(input_c3,self.Ac3)# torch.div(torch.matmul(input_c3,self.Ac3),self.l*self.h*self.w)
        input_c3 = self.relu(torch.matmul(self.Ec3,input_c3))# self.relu(torch.div(torch.matmul(self.Ec3,input_c3),self.c))
        '''######
        print('test:para:Ac1:{},Ec1:{},Sc1:{}'.format(self.Ac1,self.Ec1,self.Sc1))
        print('test:para:Ac2:{},Ec2:{},Sc2:{}'.format(self.Ac2,self.Ec2,self.Sc2))
        print('test:para:Ac3:{},Ec3:{},Sc3:{}'.format(self.Ac3,self.Ec3,self.Sc3))
        print('test for sigmoid:{}'.format(torch.matmul(self.Sc3,input_c3)))
        #exit()
        ######'''
        input_c3 = self.sigmoid(torch.matmul(self.Sc3,input_c3))# self.sigmoid(torch.div(torch.matmul(self.Sc3,input_c3),int(self.c/self.r)))
        #print('test C-3 for sigmoid:{}'.format(input_c3))
        # add
        # input_c = torch.div(input_c1,1.5) + torch.div(input_c2,1.5) + torch.div(input_c3,1.5)
        # multiply
        input_c = torch.mul(torch.mul(input_c1,input_c2),input_c3)
        input_c = torch.div(input_c,torch.mean(input_c,1,True))
        # print(input_c1,'\n',input_c2,'\n',input_c3)
        #print('c',input_c,input_c.shape),exit()
        # (b,c,1)->(b,c,1,1,1)
        input_c = input_c.unsqueeze(-1).unsqueeze(-1)
        if input_c.shape != (self.b, self.c, 1, 1, 1):
            raise ValueError('the shape of channel data is error !')
        ...
        input_s1 = self.relu(torch.matmul(self.Es1,input_s1))# self.relu(torch.div(torch.matmul(self.Es1,input_s1),self.h*self.w))
        input_s1 = self.sigmoid(torch.matmul(self.Ss1,input_s1))# self.sigmoid(torch.div(torch.matmul(self.Ss1,input_s1),self.l*self.h*self.w))
        #print('test S-1 for sigmoid:{}'.format(input_s1))
        #input_s1 = input_s1.squeeze(-1).unsqueeze(1).unsqueeze(1).reshape([self.b,1,1,self.h,self.w])
        # S2 (b,hw,l,c)->(b,hw,c)->(b,hw,c){*As2(c,1)}->{Es2(lhw,hw)*}(b,hw,1)->{Ss2(hw,lhw)*}(b,lhw,1)->(b,1,1,h,w)
        input_s2 = torch.mean(input_s, 2, False)#(b,hw,c)
        input_s2 = torch.matmul(input_s2,self.As2)# torch.div(torch.matmul(input_s2,self.As2),self.c)
        input_s2 = self.relu(torch.matmul(self.Es2,input_s2)) #self.relu(torch.div(torch.matmul(self.Es2,input_s2),self.h*self.w))
        input_s2 = self.sigmoid(torch.matmul(self.Ss2,input_s2))# self.sigmoid(torch.div(torch.matmul(self.Ss2,input_s2),self.l*self.h*self.w))
        #print('test S-2 for sigmoid:{}'.format(input_s1))
        #input_s2 = input_s2.squeeze(-1).unsqueeze(1).unsqueeze(1).reshape([self.b,1,1,self.h,self.w])
        # S3 (b,c,l,h,w)->(b,hw,lc)->(b,hw,lc){*As3(lc,1)}->{Es3(lhw,hw)*}(b,hw,1)->{Ss3(hw,lhw)*}(b,lhw,1)->(b,1,1,h,w)
        input_s3 = sta_input.reshape([self.b,self.c*self.l,self.h*self.w]).transpose(1,2)#(b,hw,lc)
        input_s3 = torch.matmul(input_s3,self.As3)# torch.div(torch.matmul(input_s3,self.As3),self.l*self.c)
        input_s3 = self.relu(torch.matmul(self.Es3,input_s3))# self.relu(torch.div(torch.matmul(self.Es3,input_s3),self.h*self.w))
        '''######
        print('test:para:As1:{},Es1:{},Ss1:{}'.format(self.As1,self.Es1,self.Ss1))
        print('test:para:As2:{},Es2:{},Ss2:{}'.format(self.As2,self.Es2,self.Ss2))
        print('test:para:As3:{},Es3:{},Ss3:{}'.format(self.As3,self.Es3,self.Ss3))
        print('test for sigmoid:{}'.format(torch.matmul(self.Ss3,input_s3)))
        exit()
        ######'''
        input_s3 = self.sigmoid(torch.matmul(self.Ss3,input_s3))# self.sigmoid(torch.div(torch.matmul(self.Ss3,input_s3),self.l*self.h*self.w))
        #print('test S-3 for sigmoid:{}'.format(input_s3))
        #input_s3 = input_s3.squeeze(-1).unsqueeze(1).unsqueeze(1).reshape([self.b,1,1,self.h,self.w])
        # add
        # input_s = torch.div(input_s1,1.5) + torch.div(input_s2,1.5) + torch.div(input_s3,1.5)
        # multiply
        input_s = torch.mul(torch.mul(input_s1,input_s2),input_s3) # (b,1,1,h,w)
        input_s = torch.div(input_s,torch.mean(input_s,1,True)).reshape([self.b,1,1,self.h,self.w])
        print('s',input_s),exit()
        if input_s.shape != (self.b, 1, 1, self.h, self.w):
            raise ValueError('the shape of spatial data is error !')
