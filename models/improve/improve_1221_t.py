...
class sta(nn.Module):
  ...
  def __init__(self,input_shape,r=256):  # r=32 or 256
    ...
    self.conv_2D_t1_1.weight=Parameter(torch.Tensor(np.divide(np.ones((1,self.h*self.w,1,1)),self.h*self.w)))
    ...
    self.conv_2D_t1_2.weight=Parameter(torch.Tensor(np.divide(np.ones((self.l,1,self.l,self.c)),self.l*self.c)))
    self.conv_2D_t2_1=torch.nn.Conv2d(in_channels=self.c,out_channels=self.c//self.r,kernel_size=1, stride=1, padding=0, dilation=1, groups=1, bias=False)
    self.conv_2D_t2_1.weight=Parameter(torch.Tensor(np.divide(np.ones((self.c//self.r,self.c,1,1)),self.c)))
    ...
    self.conv_2D_t2_2.weight=Parameter(torch.Tensor(np.divide(np.ones((self.l,self.c//self.r,self.l,self.h*self.w)),(self.c//self.r)*self.l*self.h*self.w)))
    ...
    self.conv_2D_t3_1.weight=Parameter(torch.Tensor(np.divide(np.ones((1,self.c,1,1)),self.c)))
    ...
    self.conv_2D_t3_2.weight=Parameter(torch.Tensor(np.divide(np.ones((self.l,1,self.l,self.h*self.w)),self.l*self.h*self.w)))
    ...
    self.conv_2D_t_all.weight=Parameter(torch.Tensor(np.divide(np.ones((self.l,self.l*3,1,1)),self.l*3.0)))
    ...
    # self.conv_2D_s1_1.weight=Parameter(torch.Tensor(np.divide(np.ones((1,self.c,1,1)),self.c/1.8)))
    ...
    # self.conv_2D_s2_1=torch.nn.Conv2d(in_channels=self.c,out_channels=self.c//self.r,kernel_size=1, stride=1, padding=0, dilation=1, groups=1, bias=False)
    # self.conv_2D_s2_1.weight=Parameter(torch.Tensor(np.divide(np.ones((self.c//self.r,self.c,1,1)),self.c/1.8)))
  ...
  def forward(self,sta_input: Tensor)-> Tensor:
    ...
    if self.serial:
        ...
        input_t = torch.mul(torch.mul(2.0*input_t1,2.0*input_t2),2.0*input_t3)
        ...
        att=input_t#torch.mul(input_t,input_s)
        #print('t:',input_t)
        #print('s:',input_s)
        #print('all:',att)
        #exit()
