...
class sta(nn.Module):
  ...
  def __init__(self,input_shape,r=256):  # r=32 or 256
    ...
    self.Ac1 = torch.nn.Parameter(torch.FloatTensor(nn.init.constant_(torch.Tensor(self.l,1),1.0/self.l)), requires_grad=True)# input:0.1~,1*I*l=0.1
    self.Ac2 = torch.nn.Parameter(torch.FloatTensor(nn.init.constant_(torch.Tensor(self.h*self.w,1),0.5/(self.h*self.w))), requires_grad=True)# input:0.5~
  ...
  def forward(self,sta_input: Tensor)-> Tensor:
    ...
    if not self.serial:
        ...
        input_t1 = torch.max(input_t, 3)[0]#(b,l,c)
        ...
        input_t2 = torch.max(input_t, 2)[0]#(b,l,hw)
        ...
        input_c1 = torch.max(input_c, 3)[0]#(b,c,l)
        ...
        input_c2 = torch.max(input_c, 2)[0]#(b,c,hw)
        ...
        # add
        # input_c = input_c1 + input_c2 + input_c3
        # multiply
        ...
        input_s1 = torch.max(input_s, 3)[0]#(b,hw,l)
        ...
        input_s2 = torch.max(input_s, 2)[0]#(b,hw,c)
