...
class sta(nn.Module):
  ...
  def forward(self,sta_input: Tensor)-> Tensor:
    ...
    if not self.serial:
        ...
        # multiply
        input_t = torch.add(torch.add(input_t1,input_t2),input_t3)
        ...
        # multiply
        input_c = torch.add(torch.add(input_c1,input_c2),input_c3)
        ...
        # multiply
        input_s = torch.add(torch.add(input_s1,input_s2),input_s3)#.reshape([self.b,1,1,self.h,self.w]) # (b,1,1,h,w)
