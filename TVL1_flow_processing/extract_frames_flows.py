  ...
  for ic,cls in enumerate(class_list):
    ...
    for v in tqdm(vlist):
        ...
        try:  
            ...
            # extract frames
            os.system('ffmpeg -i "%s" -r 24 -q:v 2 -vf "scale=%s" "%s"  > /dev/null 2>&1'%( os.path.join(vid_dir, cls, v), resize_str, os.path.join(outdir, '%05d.jpg')))
            ...
            # extract flows 
            os.system('export CUDA_VISIBLE_DEVICES=%d; /home/eed-server3/Downloads/MARS/MARS-master/utils1/tvl1_videoframes "%s" %d "%s" '%(gpu, os.path.join(outdir, '%05d.jpg'), nframes, os.path.join(outdir,'%05d_flow256crop.flo') ))
