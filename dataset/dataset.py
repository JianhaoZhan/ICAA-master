...
def get_test_video(opt, frame_path, Total_frames):
    ...
    if opt.modality == 'RGB': 
        while len(clip) < max(opt.sample_duration, Total_frames):
            try:
                im = Image.open(frame_path+'/%05d.jpg'%(i+1))
                clip.append(im.copy())
                im.close()
            except:
                pass
        ...
        while len(clip) < 2*max(opt.sample_duration, Total_frames):
            try:
                im_x = Image.open(frame_path+'/TVL1jpg_x_%05d.jpg'%(i+1))
                im_y = Image.open(frame_path+'/TVL1jpg_y_%05d.jpg'%(i+1))
...
def get_val_video(opt, frame_path, Total_frames):
    """
    ...
    '''  
    Total_frames:179, frame_path:/home/eed-server3/Downloads/UCF-101/Biking/v_Biking_g21_c07
'''
    if Total_frames <= opt.sample_duration: 
        loop = 1
        start_frame = int(Total_frames/4)
    else:
        start_frame = int((Total_frames - opt.sample_duration)/2)
    # choosing a random frame
    if opt.modality == 'RGB':
        while len(clip) < opt.sample_duration:
            try:
                im = Image.open(frame_path+'/%05d.jpg'%(start_frame+i+1))
                clip.append(im.copy())
                im.close()
            except:
                pass
        ...
        while len(clip) < 2*opt.sample_duration:
            try:
                im_x = Image.open(frame_path+'/TVL1jpg_x_%05d.jpg'%(start_frame+i+1))
                im_y = Image.open(frame_path+'/TVL1jpg_y_%05d.jpg'%(start_frame+i+1))
                clip.append(im_x.copy())
                clip.append(im_y.copy())
                im_x.close()
                im_y.close()
...
'''
    if Total_frames <= opt.sample_duration: 
        loop = 1
        start_frame = np.random.randint(0, int(Total_frames/2))
    else:
...
class HMDB51_test(Dataset):
    ...
    def __init__(self, train, opt, split=None):
        ...
        print(self.N,self.train_val_test )
        assert self.N == 51
        ...
        for file in split_lab_filenames:
            ...
            for line in f: 
                ...
                # Elif validation/test data        
                    ...
                    frame_path = os.path.join(opt.frame_dir, class_id, line.split(' ')[0][:-4])
                    # /home/eed-server3/Downloads/hmdb51/wave/HP_PRISONER_OF_AZKABAN_wave_u_nm_np1_ri_med_39
                    if opt.only_RGB and os.path.exists(frame_path):
                        self.data.append((line.split(' ')[0][:-4], class_id))
                    elif os.path.exists(frame_path):# and "done" in os.listdir(frame_path):
                        self.data.append((line.split(' ')[0][:-4], class_id))
    ...
    def __getitem__(self, idx):
        ...
        if self.train_val_test == 0: 
            ...
            clip = get_val_video(self.opt, frame_path, Total_frames)
        else:
...
class UCF101_test(Dataset):
    ...
    def __init__(self, train, opt, split=None):
        ...
        # indexes for training/test set
        # split_lab_filenames :['testlist01.txt', 'trainlist01.txt']
        split_lab_filenames = sorted([file for file in os.listdir(opt.annotation_path) if file.strip('.txt')[-1] ==str(split)])
    ...
    def __getitem__(self, idx):
        ...
        # frame_path = os.path.join('/home/eed-server3/Downloads/MARS/MARS-master/', video[0])
        frame_path = video[0]
