...
import models.serial_model.model_zjh as att
import models.serial_model.model_org as org
...
if __name__=="__main__":
    ...
    if opt.log == 1:
        if opt.pretrain_path != '':
            epoch_logger = Logger_MARS(os.path.join(log_path, 'tra.log')
                            ,['epoch', 'loss', 'loss_MSE', 'loss_MARS', 'acc', 'lr'], opt.MARS_resume_path, opt.begin_epoch)
            val_logger   = Logger_MARS(os.path.join(log_path, 'val.log')
    ...
    model_MARS, parameters_MARS = org.generate_model(opt)
    ...
    model_Flow, parameters_Flow = att.generate_model(opt)
    ...
    scheduler = lr_scheduler.MultiStepLR(optimizer, milestones=[9,23], gamma=0.1,  last_epoch=-1)
    ...
    for epoch in range(opt.begin_epoch, opt.n_epochs + 1):
        ...
        if epoch % opt.checkpoint == 0 and epoch >9:
            if opt.pretrain_path != '':
                save_file_path = os.path.join(log_path, 'varLR{}.pth'.format(epoch))
            else:
                save_file_path = os.path.join(log_path, 'varLR{}.pth'.format(epoch))
            states = {
                'epoch': epoch + 1,
